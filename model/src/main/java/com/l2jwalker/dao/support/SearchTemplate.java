...
import com.l2jwalker.util.SearchForm.AliasInfo;
...
public class SearchTemplate implements Serializable {
    ...
    private Map<String, Object> parameters = new FastMap<String, Object>();

    private List<OrderBy> orderBys = new FastList<OrderBy>();
    ...
    // date ranges
    private Map<String, DateRange<?>> dateRanges = new FastMap<String, DateRange<?>>();
    // raw Criterions
    private List<Criterion> criterions = new FastList<Criterion>();
    ...
    /**
     * Constructs a new search data and initializes it with the values of the passed search data.
     */
    public SearchTemplate(SearchTemplate searchTemplate) {
        ...
        for (OrderBy ob : searchTemplate.getOrderBys()) {
            addOrderBy(ob); // id by is read-only, we can use the same
        }
        ...
        for (DateRange<?> dr : searchTemplate.getDateRanges()) {
            ...
            DateRange copy = new DateRange(dr);
            add(copy); // date range is not readonly, we must clone it
        }
    ...
    public SearchTemplate addParameter(String parameterName, Object parameterValue) {
        if (parameters == null) {
            parameters = new FastMap<String, Object>();
        }
    ...
    public SearchTemplate setOrderDesc(boolean desc) {
        int orderBySize = orderBys.size();
        if (orderBySize == 0) {
            orderBys.add(new OrderBy("", desc ? OrderByDirection.DESC : OrderByDirection.ASC));
        } else if (orderBySize == 1) {
            OrderBy orderBy = orderBys.get(0);
            orderBys.clear();
            orderBys.add(new OrderBy(orderBy.getColumn(), desc ? OrderByDirection.DESC : OrderByDirection.ASC));
        } else {
            throw new IllegalStateException("Please use addOderBy");
        }
        return this;
    ...
    /**
     * When not null, indicates if the DAO layer should cache or not the search results. If null is returned, it is up
     * to the DAO layer to decide what to do.
     */
    ...
    public SearchTemplate setDateRanges(Collection<DateRange<?>> dateRanges) {
        this.dateRanges = new FastMap<String, DateRange<?>>();
    ...
    /**
     * Add a criterion. It allows you to add NullCriterion on x-to-one relation. For example
     * add(Restrictions.isNull("linkedEntity"))
     */
    ...
    public void addAlias(List<AliasInfo> aliasInfo) {
